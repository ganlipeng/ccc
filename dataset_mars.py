...
class TSNDataSet(data.Dataset):
    def __init__(self, root_path, list_file,
                 ...
                 image_tmpl='img_{:05d}.jpg', transform=None, transform_flow=None,
    ...
    def _load_image(self, directory, idx):
        if self.modality == 'RGB' or self.modality == 'RGBDiff':
            ...
            if self.image_tmpl == 'TVL1jpg_{}_{:05d}.jpg':  # ucf
        ...
        if self.dense_sample:  # i3d dense sample
            ...
            if record.num_frames > self.num_segments + self.new_length - 1:
                if self.modality == 'RGB':
                    ...
                    tick = (record.num_frames-1 - self.new_length + 1) / float(self.num_segments)
    ...
    def __getitem__(self, index):
        ...
        if self.image_tmpl == 'TVL1jpg_{}_{:05d}.jpg':
        ...
        image_tensor, target = self.get(record, segment_indices)
        
        if self.random_shift:
            self.modality = 'Flow'
            self.image_tmpl = 'TVL1jpg_{}_{:05d}.jpg'
            self.new_length = 5
            segment_indices_flow = self._sample_indices(record)     
            flow_tensor = self.get_flow(record, segment_indices_flow)
            self.modality = 'RGB'
            self.image_tmpl = '{:05d}.jpg'
            self.new_length = 1
            return image_tensor, flow_tensor, target
        else:
    ...
    def get_flow(self, record, indices):
        ...
        for seg_ind in indices:
            ...
            for i in range(5):
                ...
                if p < record.num_frames:
                    p += 1
        process_data = self.transform_flow(images)
        return process_data
