...
class TSN_mars(nn.Module):
    ...
    def train(self, mode=True):
        ...
        :return:
        """
        super(TSN_mars, self).train(mode)
        count = 0
    ...
    def forward(self, input, no_reshape=False):
      ...
      #      base_out = self.base_model(input.view((-1, sample_len) + input.size()[-2:]))
            ...
            x1 = self.base_model.maxpool(self.base_model.relu(self.base_model.bn1(self.base_model.conv1(x))))
            
            x2 = self.base_model.layer1(x1)
            x3 = self.base_model.layer2(x2)
            x4 = self.base_model.layer3(x3)
            x5 = self.base_model.layer4(x4)
            x6 = self.base_model.avgpool(x5)
            x6 = x6.view(-1,2048)
            output_avg = x6.view((-1, self.num_segments) + x6.size()[1:])
            output_avg = output_avg.mean(dim=1, keepdim=True)
            base_out = self.base_model.fc(x6)
    ...
    #        print(base_out.shape)
        ...
        #    return output.squeeze(1)
            return [output.squeeze(1), output_avg.squeeze(1)]
